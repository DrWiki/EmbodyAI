...
class ArmController:
    def __init__(self, motor_control=None):
        ...
        self.Motor6 = None
        self.Motor7 = None
        self.Motor_control = motor_control if motor_control else None  # 接受外部传入的MotorControl对象

        self.motor_cmd_velocities = [0.3] * 7
        self.arm_type = None

    def set_speed(self, speed: float):
        self.motor_cmd_velocities = np.ones([7]) * speed

    def Motor_Init(self, motor_control, arm_type):
        self.arm_type = arm_type
        self.Motor_control = motor_control

        # 根据arm_type初始化不同的电机信息
        if arm_type == 'left':
            motors_info = [
        ...
        print(f"{arm_type.capitalize()} Arm Initialized Successfully")
...
class BodyController:
    def __init__(self, motor_control=None):
        ...
        self.Motor3 = None
        self.Motor4 = None
        self.Motor_control = motor_control  # 使用传入的motor_control对象

        self.motor_cmd_velocities = [0.2] * 4
        self.body_type = None

    def set_speed(self, speed: float):
        self.motor_cmd_velocities = np.ones([4]) * speed

    def Motor_Init(self, motor_control, body_type):
